    ...
    <div className="min-h-screen text-white font-sans overflow-x-hidden" style={{backgroundColor:"#020a0f"}}>
      ...
      <section className="relative min-h-screen flex items-center pt-20 pb-16 overflow-hidden">
        ...
        <motion.div className="absolute inset-0 pointer-events-none"
          animate={{background:[
            `radial-gradient(ellipse at 20% 30%, ${ACCENT_HEX}20 0%, #020a0f 60%)`,
            `radial-gradient(ellipse at 80% 70%, ${ACCENT_HEX}14 0%, #020a0f 60%)`,
              ...
              {frame.headline.split("\n").map((line: string, i: number) => (
                <motion.span key={`${frameIdx}-${i}`} className={`block text-6xl md:text-7xl ${
                  i === frame.headline.split("\n").length - 1 && frame.headline.split("\n").length > 1
                    ? "bg-gradient-to-r from-cyan-600 to-blue-500 bg-clip-text text-transparent italic"
                    : "text-white"
                }`}
