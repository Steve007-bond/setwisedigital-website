...
const TOOLS: Tool[] = [
  { href: "/tools/smart-home-matcher", emoji: "🏠", label: "Best Match", labelBg: "bg-amber-600", title: "Smart Home Starter Matcher", desc: "Alexa, Google Nest, or Apple HomeKit? 5 questions match the right system to your lifestyle.", tags: ["Alexa", "Google", "HomeKit"], gradient: "from-amber-500 to-orange-400" },
  { href: "/tools/voice-assistant-matcher", emoji: "🎙️", label: "Compare", labelBg: "bg-purple-600", title: "Best Voice Assistant for You", desc: "Alexa vs Google Assistant vs Siri — 5 questions match you to the best voice assistant.", tags: ["Alexa", "Google", "Siri"], gradient: "from-violet-500 to-purple-400" },
  { href: "/tools/wifi-checker", emoji: "📶", label: "Check Wi-Fi", labelBg: "bg-sky-600", title: "Home Wi-Fi Overload Checker", desc: "Find out if your router can handle all your smart devices — and what to do if not.", tags: ["Wi-Fi", "Router", "Devices"], gradient: "from-sky-500 to-blue-400" },
  { href: "/tools/subscription-audit", emoji: "📊", label: "Save Money", labelBg: "bg-teal-600", title: "Tech Subscription Audit", desc: "Find forgotten subscriptions, cancel what you don't need, and see your true monthly cost.", tags: ["Netflix", "Amazon", "Save"], gradient: "from-teal-500 to-cyan-400" },
  { href: "/tools/home-security-audit", emoji: "🔒", label: "Stay Safe", labelBg: "bg-red-600", title: "Home Security Audit", desc: "10 questions reveal your home's security rating A–F and what to improve first.", tags: ["Ring", "SimpliSafe", "ADT"], gradient: "from-red-500 to-rose-400" },
  { href: "/tools/best-printer-finder", emoji: "🖨️", label: "Find Yours", labelBg: "bg-indigo-600", title: "Best Printer Finder", desc: "5 questions match you to the perfect printer for your home and budget.", tags: ["HP", "Canon", "Epson"], gradient: "from-indigo-500 to-blue-400" }
];

const ISSUES = [
  ["🔗", "Account Linking", "Connect Spotify, Amazon Music, and your calendar to Alexa in minutes."],
  ["🎓", "Voice Training", "Teach Alexa to understand your voice clearly — fewer misheard commands."],
  ["🏠", "Smart Home Control", "Control lights, plugs, and cameras by voice — even for beginners."],
  ["⏰", "Reminders & Routines", "Set medication reminders, morning routines, and daily schedules with your voice."]
];

const STATS = [
  ["50+", "useful Alexa commands most Echo owners never discover"],
  ["65%", "of adults 50+ own a smart speaker (Pew Research 2024)"],
  ["5 min", "to set up your first Alexa daily reminder or routine"]
];
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 36 }} animate={inView ? { opacity: 1, y: 0 } : {}}
      ...
      <Link href={tool.href} className="block h-full">
        <div className="h-full bg-zinc-900 border border-zinc-800 hover:border-cyan-500/60 rounded-3xl overflow-hidden transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-black/40 group">
          ...
          <div className="p-7">
            ...
            <h3 className="text-xl font-black text-white mb-3 leading-snug group-hover:text-cyan-300 transition-colors">{tool.title}</h3>
            ...
            <div className="flex items-center gap-2 text-sm font-black text-cyan-300 group-hover:gap-3 transition-all">Try Free <ArrowRight size={14} /></div>
          </div>
        </div>
      </Link>
    </motion.div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Margaret"
            className="w-full pl-11 pr-4 py-4 bg-zinc-800 border border-zinc-700 hover:border-zinc-600 focus:border-[#06b6d4] rounded-2xl text-white text-lg placeholder:text-zinc-600 font-medium outline-none transition-colors" />
        </div>
...
export default function PageClient() {
  return (
    <div className="min-h-screen text-white font-sans overflow-x-hidden" style={{ backgroundColor: "#050a0a" }}>
      <Navbar /><ScrollToTop />

      {/* HERO */}
      <section className="relative min-h-screen flex items-center pt-20 pb-16 overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center opacity-[0.07]" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1543512214-318c7553f230?auto=format&fit=crop&q=50&w=700')" }} />
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-950/60 via-[#050a0a]/90 to-[#050a0a]" />
        {[...Array(14)].map((_,i) => (
          <motion.div key={i} className="absolute rounded-full opacity-20"
            style={{ width:`${6+(i%4)*3}px`, height:`${6+(i%4)*3}px`, left:`${(i*19+5)%100}%`, top:`${(i*27+8)%100}%`, backgroundColor: "#06b6d4" }}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-cyan-500/10 border-cyan-500/25 text-cyan-300 text-sm font-black uppercase tracking-[0.14em] mb-8 border">
              Alexa & Voice Assistant Mastery
            </motion.div>
            <h1 className="font-black leading-[0.92] tracking-tighter mb-8">
              <motion.span className="block text-7xl md:text-8xl text-white" initial={{opacity:0,y:50}} animate={{opacity:1,y:0}} transition={{delay:0.15,type:"spring",stiffness:80}}>Your Voice,</motion.span>
              <motion.span className="block text-7xl md:text-8xl from-cyan-600 to-blue-500 bg-gradient-to-r bg-clip-text text-transparent italic" initial={{opacity:0,y:50}} animate={{opacity:1,y:0}} transition={{delay:0.24,type:"spring",stiffness:80}}>Your Home,</motion.span>
              <motion.span className="block text-7xl md:text-8xl text-white" initial={{opacity:0,y:50}} animate={{opacity:1,y:0}} transition={{delay:0.32999999999999996,type:"spring",stiffness:80}}>Simplified.</motion.span>
            </h1>
            <motion.p className="text-xl md:text-2xl text-zinc-300 font-medium mb-3 leading-relaxed max-w-lg"
              initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.55 }}>
              Learn 50+ Alexa commands, set up smart home routines, and control your whole home with just your voice.
            </motion.p>
            <motion.p className="text-base text-zinc-500 mb-10 max-w-lg"
              initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.65 }}>
              Designed for adults 45+ who want to use their Echo Dot or Echo Show with real confidence.
            </motion.p>
            <motion.div className="flex flex-col sm:flex-row gap-4 mb-10"
              ...
              <motion.a href="#learn" whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }}
                className="px-9 py-5 bg-gradient-to-r from-cyan-600 to-blue-500 text-white font-black text-xl rounded-2xl flex items-center justify-center gap-3 shadow-xl shadow-cyan-900/50">
                Get My Alexa Learning Guide <ArrowRight size={20} />
              </motion.a>
              <Link href="/contact">
                <motion.div whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }}
                  className="px-9 py-5 border-2 border-zinc-700 hover:border-[#06b6d4] text-white font-black text-xl rounded-2xl flex items-center justify-center gap-3 transition-colors cursor-pointer">
                  Book a Live Lesson
                </motion.div>
              </Link>
            </motion.div>
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.9 }}
              ...
              <div className="flex items-center gap-2 text-zinc-400 text-sm"><CheckCircle2 size={14} className="text-cyan-300" /> Plain English</div>
            </motion.div>
          </div>
          <motion.div initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.3 }} className="hidden lg:block">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-black/60 border border-zinc-800">
              <img src="https://images.unsplash.com/photo-1543512214-318c7553f230?auto=format&fit=crop&q=50&w=700" alt="Technology learning guide" className="w-full h-[500px] object-cover" />
              ...
              <motion.div animate={{ y:[0,-8,0] }} transition={{ duration:3, repeat:Infinity }}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-600 to-blue-500 flex items-center justify-center text-xl">🎙️</div>
                  <div>
                    <div className="text-white font-black text-base">Alexa Setup</div>
                    <div className="text-zinc-400 text-sm">Echo Dot, Echo Show, Echo</div>
                  </div>
                </div>
              </motion.div>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {STATS.map((s,i) => (
            <motion.div key={i} initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ delay:i*0.1 }}
              ...
              <span className="text-4xl font-black tabular-nums text-cyan-400">{s[0]}</span>
      ...
      <section className="py-24 bg-zinc-950/50 border-y border-zinc-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} className="text-center mb-16">
            ...
            <p className="text-zinc-400 text-lg">Everything you need to get full value from your Alexa device</p>
          </motion.div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {ISSUES.map((issue,i) => (
              <motion.div key={i} initial={{ opacity:0, y:40 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
                ...
                className="bg-zinc-900 border border-zinc-800 hover:border-cyan-500/60 rounded-3xl p-8 transition-all duration-300 group">
                <div className="text-5xl mb-6">{issue[0]}</div>
                <h3 className="text-xl font-black text-white mb-3 group-hover:text-cyan-300 transition-colors">{issue[1]}</h3>
      ...
      {/* RELATED TOOLS */}
      <section id="tools" className="py-28 border-t border-zinc-800/50" style={{ backgroundColor:"#050a0a" }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity:0, y:30 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border text-sm font-black uppercase tracking-widest mb-6"
              style={{ backgroundColor:"#06b6d4"+"15", borderColor:"#06b6d4"+"30", color:"#06b6d4" }}>
              <Zap size={14} /> 6 Free Smart Home Tools
            </div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-5 text-white">
              Free Tools — Alexa &<br className="hidden md:block" />
              <span className="bg-gradient-to-r from-cyan-400 to-blue-300 bg-clip-text text-transparent">Smart Home Setup</span>
            </h2>
            <p className="text-zinc-400 text-xl max-w-2xl mx-auto leading-relaxed">Each tool helps you get more from your Alexa device and smart home setup — step by step.</p>
          </motion.div>
          ...
          <div className="text-center">
            <Link href="/tools">
              <motion.div whileHover={{ scale:1.03 }} className="inline-flex items-center gap-3 px-10 py-5 border-2 border-zinc-700 hover:border-[#06b6d4] text-white font-black text-lg rounded-2xl transition-colors cursor-pointer">
      ...
      <section className="py-28 relative overflow-hidden border-t border-zinc-800/50">
        <div className="absolute inset-0 opacity-[0.04] bg-cover bg-center" style={{ backgroundImage:"url('https://images.unsplash.com/photo-1543512214-318c7553f230?auto=format&fit=crop&q=50&w=700')" }} />
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-950/60 via-[#050a0a]/95 to-[#050a0a]" />
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity:0, x:-30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}>
            ...
            <div className="space-y-5">
              <motion.div key={0} initial={{opacity:0,x:-20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{delay:0.0}} className="flex items-start gap-4"><span className="text-2xl flex-shrink-0 mt-0.5">🎙️</span><span className="text-zinc-300 text-lg leading-relaxed">50+ useful Alexa commands explained in plain English</span></motion.div>
              <motion.div key={1} initial={{opacity:0,x:-20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{delay:0.1}} className="flex items-start gap-4"><span className="text-2xl flex-shrink-0 mt-0.5">⏰</span><span className="text-zinc-300 text-lg leading-relaxed">How to set up Alexa routines for reminders and daily life</span></motion.div>
              ...
              <motion.div key={3} initial={{opacity:0,x:-20}} whileInView={{opacity:1,x:0}} viewport={{once:true}} transition={{delay:0.30000000000000004}} className="flex items-start gap-4"><span className="text-2xl flex-shrink-0 mt-0.5">🔧</span><span className="text-zinc-300 text-lg leading-relaxed">Fix the most common Alexa problems easily at home</span></motion.div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity:0, x:30 }} whileInView={{ opacity:1, x:0 }} viewport={{ once:true }}>
      ...
      <section className="py-20 border-t border-zinc-800/50 bg-zinc-950/40">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <Link href="/contact">
            <motion.div whileHover={{ scale:1.03 }} className="inline-flex items-center gap-3 px-10 py-5 bg-gradient-to-r from-cyan-600 to-blue-500 text-white font-black text-xl rounded-2xl shadow-xl shadow-cyan-900/40 cursor-pointer">
