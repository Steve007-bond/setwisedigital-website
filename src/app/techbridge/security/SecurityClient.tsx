...
function SecuritySimulator() {
  ...
  const scene = SECURITYSIMULATOR_SCENES[sceneIdx];
  const progress = ((sceneIdx + 1) / SECURITYSIMULATOR_SCENES.length) * 100;

  useEffect(() => {
    if (!isPlaying) return;
    const timer = setTimeout(() => {
      if (sceneIdx < SECURITYSIMULATOR_SCENES.length - 1) {
        setSceneIdx(s => s + 1);
      } else {
        setIsPlaying(false);
      }
    ...
    <section ref={simRef} className="py-20 border-b border-zinc-800/50 bg-zinc-950/60 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-3">See How Online Security Works</h2>
        ...
        <motion.div initial={{opacity:0,scale:0.95}} animate={isVisible ? {opacity:1,scale:1} : {}}
          ...
          <div className="relative bg-zinc-900/80 border border-zinc-800 rounded-3xl p-6 md:p-10 backdrop-blur-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="ml-3 text-xs text-zinc-500 font-bold">See How Online Security Works</span>
            </div>
            ...
            <div className="relative h-[220px] md:h-[260px] mb-8">
              ...
              <motion.div
                ...
                animate={{ opacity: scene.elements.email ? 1 : 0.15, scale: scene.elements.email ? 1 : 0.85 }}
                transition={{ duration: 0.6 }}
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center"
                  style={{ borderColor: scene.elements.email ? scene.color : "#374151", backgroundColor: scene.elements.email ? `${scene.color}15` : "#1e293b", transition: "all 0.6s" }}>
                  <span className="text-2xl md:text-3xl">📧</span>
                </div>
                <span className="text-[11px] text-zinc-500 font-bold mt-2">Email</span>
                {scene.elements.email && (
              ...
              <motion.div
                ...
                animate={{ opacity: scene.elements.scan ? 1 : 0.15, scale: scene.elements.scan ? 1 : 0.85 }}
                transition={{ duration: 0.6 }}
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center"
                  style={{ borderColor: scene.elements.scan ? scene.color : "#374151", backgroundColor: scene.elements.scan ? `${scene.color}15` : "#1e293b", transition: "all 0.6s" }}>
                  <span className="text-2xl md:text-3xl">🔍</span>
                </div>
                <span className="text-[11px] text-zinc-500 font-bold mt-2">Scan</span>
                {scene.elements.scan && (
                  <motion.div className="absolute -top-1 -right-1 w-3 h-3 rounded-full"
                    style={{ backgroundColor: scene.color }}
                    animate={{ scale: [1, 1.4, 1] }}
              ...
              <motion.div
                ...
                animate={{ opacity: scene.elements.check ? 1 : 0.15, scale: scene.elements.check ? 1 : 0.85 }}
                transition={{ duration: 0.6 }}
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center"
                  style={{ borderColor: scene.elements.check ? scene.color : "#374151", backgroundColor: scene.elements.check ? `${scene.color}15` : "#1e293b", transition: "all 0.6s" }}>
                  <span className="text-2xl md:text-3xl">🛡️</span>
                </div>
                <span className="text-[11px] text-zinc-500 font-bold mt-2">Antivirus</span>
                {scene.elements.check && (
                  <motion.div className="absolute -top-1 -right-1 w-3 h-3 rounded-full"
                    style={{ backgroundColor: scene.color }}
                    animate={{ scale: [1, 1.4, 1] }}
                    transition={{ duration: 1, repeat: Infinity }} />
              ...
              <motion.div
                ...
                animate={{ opacity: scene.elements.block ? 1 : 0.15, scale: scene.elements.block ? 1 : 0.85 }}
                transition={{ duration: 0.6 }}
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center"
                  style={{ borderColor: scene.elements.block ? scene.color : "#374151", backgroundColor: scene.elements.block ? `${scene.color}15` : "#1e293b", transition: "all 0.6s" }}>
                  <span className="text-2xl md:text-3xl">🚫</span>
                </div>
                <span className="text-[11px] text-zinc-500 font-bold mt-2">Block</span>
              ...
              <motion.div
                ...
                animate={{ opacity: scene.elements.safe ? 1 : 0.15, scale: scene.elements.safe ? 1 : 0.85 }}
                transition={{ duration: 0.6 }}
              >
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl border-2 flex items-center justify-center"
                  style={{ borderColor: scene.elements.safe ? scene.color : "#374151", backgroundColor: scene.elements.safe ? `${scene.color}15` : "#1e293b", transition: "all 0.6s" }}>
                  <span className="text-2xl md:text-3xl">🔑</span>
                </div>
                <span className="text-[11px] text-zinc-500 font-bold mt-2">2FA</span>
                {scene.elements.safe && (
                  <motion.div className="absolute -top-1 -right-1 w-3 h-3 rounded-full"
                    style={{ backgroundColor: scene.color }}
                    animate={{ scale: [1, 1.4, 1] }}
              ...
              {scene.elements.email && scene.elements.scan && (
              ...
              {scene.elements.scan && scene.elements.check && (
              ...
              {scene.elements.check && scene.elements.block && (
              ...
              {scene.elements.block && scene.elements.safe && (
            ...
            <div className="flex justify-center gap-1.5 mb-6">
              {SECURITYSIMULATOR_SCENES.map((_: typeof scene, i: number) => (
            ...
            <div className="flex justify-center gap-3">
              {!hasStarted ? (
                ...
                <>
                  ...
                  {sceneIdx < SECURITYSIMULATOR_SCENES.length - 1 && (
                    <button onClick={() => setSceneIdx(s => Math.min(s + 1, SECURITYSIMULATOR_SCENES.length - 1))}
      ...
      <section className="relative min-h-screen flex items-center pt-20 pb-16 overflow-hidden">
        ...
        <div className="absolute inset-0">
          <motion.div className="absolute inset-0 pointer-events-none"
            ...
            style={{ background: `radial-gradient(ellipse at 30% 40%, ${frame.color}18 0%, #0a0000 55%), radial-gradient(ellipse at 70% 60%, ${frame.color}10 0%, #0a0000 55%)` }}
          />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.05}}
              ...
              ONLINE SECURITY LEARNING GUIDE
            </motion.div>
            ...
            <div className="relative lg:scale-[1.4] lg:origin-center lg:translate-x-[10%] lg:-translate-y-[3%]">
              ...
              <motion.div animate={{y:[0,-14,0]}} transition={{duration:5,repeat:Infinity,ease:"easeInOut"}}
                ...
                <div className="relative overflow-hidden">
                  <img src="/Images/hero-security.jpeg" alt="Online security guide"
              ...
              {[
                {emoji: frame.particles[0], label:"Protect", pos:"top-[6%] left-[0%]", delay:0},
                {emoji: frame.particles[1], label:"Lock", pos:"top-[10%] right-[0%]", delay:0.5},
                {emoji: frame.particles[2], label:"Keys", pos:"bottom-[30%] left-[-2%]", delay:1},
                {emoji: frame.particles[3], label:"Email", pos:"bottom-[26%] right-[-2%]", delay:1.5},
              ].map((b,i)=>(
