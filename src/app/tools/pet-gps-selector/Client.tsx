...
export default function Client() {
  ...
  const handleSubmit = async () => {
    ...
    try { await fetch("/api/leads", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ name, email, phone, issue: `Pet GPS — Interested in: ${selected?.name}`, source:"pet-gps-selector" }) }); } catch (e) { console.error("[lead] error:", e); }
                ...
                <motion.div key={tracker.id} initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:i*0.08}} className="bg-zinc-900 rounded-[2rem] border border-zinc-800 overflow-hidden">
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-5">
                      <div><div className="text-4xl mb-2">{tracker.emoji}</div><h3 className="text-2xl font-black text-white mb-2">{tracker.name}</h3><span className={`${tracker.badgeColor || "bg-blue-600"} text-white text-sm font-black px-3 py-1.5 rounded-full`}>{tracker.badge}</span></div>
