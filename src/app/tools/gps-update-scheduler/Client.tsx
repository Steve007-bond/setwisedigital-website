...
const bgs = [
  { url:"https://images.unsplash.com/photo-1526779259212-939e64788e3c?auto=format&fit=crop&q=60&w=1200", type:"image" as const, theme:"dark" as const },
...
export default function Client() {
  ...
  const handleSubmit = async () => {
    ...
    try { await fetch("/api/leads", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ name, email, phone, brand, issue: `GPS Update Scheduler — ${brand} — Last updated: ${month} ${year} — ${urgency?.label}`, source:"gps-update-scheduler" }) }); } catch (e) { console.error("[lead] error:", e); }
