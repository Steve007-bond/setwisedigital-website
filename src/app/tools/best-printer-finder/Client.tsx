...
export default function Client() {
  ...
  const handleSubmit = async () => {
    ...
    try { await fetch("/api/leads",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,email,phone,brand:topPick?.brand,issue:`Printer Finder — Top Pick: ${topPick?.name}`,source:"best-printer-finder",extra:JSON.stringify(answers)})}); } catch (e) { console.error("[lead] error:", e); }
          ...
          {stage==="results"&&topPick&&(<motion.div key="results" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-6">
            ...
            <div className="bg-zinc-900 rounded-[2rem] border-2 border-violet-800 overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div className="flex items-start justify-between gap-4 mb-6"><div><h3 className="text-3xl font-black text-white mb-2">{topPick.emoji} {topPick.name}</h3><div className="flex items-center gap-3"><span className={`${topPick.badgeColor || "bg-blue-600"} text-white text-sm font-black px-3 py-1.5 rounded-full`}>{topPick.badge}</span><span className="text-zinc-400 font-bold text-xl">{topPick.price}</span><span className="text-zinc-500 text-sm">{topPick.type}</span></div></div></div>
            ...
            <div className="bg-zinc-900 rounded-[2rem] border border-zinc-800 p-6"><h3 className="font-black text-white mb-4 text-lg">Also consider</h3><div className="space-y-3">{ranked.slice(1,3).map(p=>(<div key={p.id} className="flex items-center gap-4 p-4 bg-zinc-800 rounded-2xl"><div className="text-2xl">{p.emoji}</div><div className="flex-1"><div className="font-black text-white">{p.name}</div><div className="text-zinc-400 text-sm">{p.price} · {p.type} · {p.costPerPage}</div></div><span className={`${p.badgeColor || "bg-blue-600"} text-white text-sm font-black px-2.5 py-1 rounded-full`}>{p.badge}</span></div>))}</div></div>
