          ...
          {stage==="results"&&topPick&&(<motion.div key="results" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-6">
            ...
            <div className="bg-zinc-900 rounded-[2rem] border-2 border-purple-800 overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div className="flex items-start justify-between gap-4 mb-6"><div><h3 className="text-3xl font-black text-white mb-2">{topPick.emoji} {topPick.name}</h3><div className="flex items-center gap-3"><span className={`${topPick.badgeColor} text-white text-sm font-black px-3 py-1.5 rounded-full`}>{topPick.badge}</span><span className="text-zinc-400 font-bold text-base">{topPick.price}</span></div></div></div>
            ...
            <div className="bg-zinc-900 rounded-[2rem] border border-zinc-800 p-6"><h3 className="font-black text-white mb-4 text-lg">Also consider</h3><div className="space-y-3">{ranked.slice(1).map(a=>(<div key={a.id} className="flex items-center gap-4 p-4 bg-zinc-800 rounded-2xl"><div className="text-2xl">{a.emoji}</div><div className="flex-1"><div className="font-black text-white">{a.name}</div><div className="text-zinc-400 text-sm">{a.device} · {a.price}</div></div><span className={`${a.badgeColor} text-white text-sm font-black px-2.5 py-1 rounded-full`}>{a.badge}</span></div>))}</div></div>
