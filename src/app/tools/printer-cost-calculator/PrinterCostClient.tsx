...
export default function PrinterCostClient() {
  ...
  const handleSubmit = async () => {
    ...
    try { await fetch("/api/leads", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ name, email, phone, brand: selectedPrinter?.brand, issue: `Printer Cost Calc — ${selectedPrinter?.model} — $${costs?.total.toFixed(0)}/yr`, source:"printer-cost-calculator" }) }); } catch (e) { console.error("[lead] error:", e); }
